...
const buildSessionCookie = (value: string, expiresAt?: Date) => {
  const sessionCookieSecure = process.env.SESSION_COOKIE_SECURE?.toLowerCase() === "true";
  const sessionCookieSameSite = process.env.SESSION_COOKIE_SAMESITE?.trim() || "Lax";
...
async function startServer() {
  ...
  app.use((req, res, next) => {
    ...
    if (process.env.NODE_ENV === "production") {
      ...
      const frameAncestors = isSharedReportView
        ...
        : "'none'";
      res.setHeader("Content-Security-Policy", `default-src 'self'; img-src 'self' data: blob:; style-src 'self' 'unsafe-inline' https://fonts.googleapis.com https://fonts.maateen.me; font-src 'self' https://fonts.gstatic.com https://fonts.maateen.me; script-src 'self'; connect-src 'self'; object-src 'none'; base-uri 'self'; frame-ancestors ${frameAncestors}`);
  ...
  app.delete("/api/data-share/clients/:id", requirePermission("settings"), async (req, res) => {
    const { id } = req.params;
    const existingClient = await db.prepare("SELECT id, name, status FROM data_share_clients WHERE id = ?").get(id) as { id: number; name: string; status: string } | undefined;
    if (!existingClient) {
      return res.status(404).json({ success: false, message: "API client not found." });
    }
    if (existingClient.status === "Active") {
      return res.status(400).json({ success: false, message: "Revoke this API client before deleting it." });
    }
    await db.prepare("DELETE FROM data_share_clients WHERE id = ?").run(id);
    await writeDataShareAccessLog({
