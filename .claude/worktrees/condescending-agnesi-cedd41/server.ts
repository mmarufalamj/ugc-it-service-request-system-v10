...
const DEFAULT_SYSTEM_SETTINGS: AppSystemSettings = {
  ...
  dailyQuotes: [],
  selectedDailyQuoteIndex: undefined,
...
async function startServer() {
  ...
  app.delete("/api/data-share/clients/:id", requirePermission("settings"), async (req, res) => {
    try {
      ...
      const deleteResult = await db.prepare("DELETE FROM data_share_clients WHERE id = ?").run(id);
      ...
      res.json({ success: true, message: "API client deleted successfully." });
    } catch (error) {
      console.error("Error deleting data-share client:", error);
      res.status(500).json({ success: false, message: error instanceof Error ? error.message : "Failed to delete API client." });
    }
  });
